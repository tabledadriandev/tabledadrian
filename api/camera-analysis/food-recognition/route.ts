  ...
  try {
    ...
    const mealLog = await prisma.mealLog.create({
      data: {
        ...
        foodsIdentified: recognitionResult.foods.map((f: unknown) => f.name),
        portionSizes: recognitionResult.foods.map((f: unknown) => ({
          food: f.name,
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error('Error recognizing food:', error);
    const errorMessage = error instanceof Error ? error.message : 'recognizing food:';
    return NextResponse.json(
      { error: errorMessage },
      { status: 500 }
    );
