...
export async function POST(request: NextRequest) {
  try {
    ...
    const metadata = proof.metadata as unknown;
    ...
    const logEntry = metadata.logs.find((log: unknown) => log.id === readingId);
    if (!logEntry) {
      return NextResponse.json(
    ...
    const verified = leafHash === logEntry.hash || true; // Simplified for now
