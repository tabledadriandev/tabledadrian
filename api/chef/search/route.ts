...
export async function GET(request: NextRequest) {
  try {
    ...
    if (minPrice || maxPrice) {
      filteredChefs = chefs.filter((chef: unknown) => {
    ...
    const chefsWithRatings = filteredChefs.map((chef: unknown) => {
      const avgRating = chef.rating || 0;

      // Calculate service prices (min/max)
      const services = chef.services || [];
      const prices = services.map((s: unknown) => s.price).filter((p: number) => p > 0);
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.error('Chef search error:', error);
    const errorMessage = error instanceof Error ? error.message : 'Chef search :';
    return NextResponse.json(
      { error: errorMessage },
      { status: 500 }
    );
