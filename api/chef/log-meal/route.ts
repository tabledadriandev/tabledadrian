...
export async function POST(request: NextRequest) {
  try {
    ...
    // Calculate rewards for chef (Crypto Path: 5 $tabledadrian per verified meal)
    const chefVerified = verifyOnChain && txHash !== null;
    if (chef.acceptsCrypto && chefVerified) {
      const rewardAmount = 5; // $tabledadrian tokens
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error('Log meal error:', error);
    const errorMessage = error instanceof Error ? error.message : 'Log meal :';
    return NextResponse.json(
      { error: errorMessage },
      { status: 500 }
    );
