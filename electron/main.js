...
function createWindow() {
  ...
  if (isDev) {
    ...
    const waitForServer = () => {
      ...
      const checkServer = (port) => {
        ...
        const testUrl = `http://localhost:${port}/app/test-page`;
  ...
  mainWindow.webContents.on('will-navigate', (event, navigationUrl) => {
    ...
    // Block navigation to website routes (root, /coin, /app-download, etc.)
    // Only allow wellness app routes (/app/*)
    if (parsedUrl.origin === 'http://localhost:3000') {
      // Block navigation to root or website routes
      if (parsedUrl.pathname === '/' || 
          parsedUrl.pathname.startsWith('/coin') ||
          parsedUrl.pathname.startsWith('/app-download') ||
          (!parsedUrl.pathname.startsWith('/app') && !parsedUrl.pathname.startsWith('/api'))) {
      ...
      // Allow navigation within wellness app routes (/app/*) and API routes
      return;
    }
  ...
  mainWindow.webContents.on('did-navigate', (event, url) => {
    ...
    // If somehow we navigated to root, redirect to /app
    if (parsedUrl.pathname === '/' && parsedUrl.origin === 'http://localhost:3000') {
      console.log('Redirecting from root to /app');
      mainWindow.loadURL('http://localhost:3000/app');
    }
  });
}
